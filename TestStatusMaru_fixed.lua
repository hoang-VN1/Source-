...
-- Load GUI library FIRST (blocking, with error catch)
local guiOk, guiErr = pcall(function()
    loadstring(game:HttpGet("https://pastefy.app/gFtbfZ7n/raw"))()
end)
if not guiOk then warn("[MaruHub] GUI loader failed:", guiErr) end

-- All waits handled by wrapper - just grab refs safely
local Players = game:GetService("Players")
local plr_init = Players.LocalPlayer

-- Extra safety: wait for character + Data in case wrapper didn't fully cover it
repeat task.wait(0.1) until plr_init.Character and plr_init.Character:FindFirstChild("HumanoidRootPart")
repeat task.wait(0.1) until plr_init:FindFirstChild("Data") and plr_init.Data:FindFirstChild("Level")
repeat task.wait(0.1) until
    plr_init.PlayerGui:FindFirstChild("Main") and
    plr_init.PlayerGui.Main:FindFirstChild("Loading") and
    not plr_init.PlayerGui.Main.Loading.Visible
...
task.spawn(function()
    local ok, err = pcall(function()
        loadstring(game:HttpGet("https://pastefy.app/wTNqqh25/raw"))()
    end)
    if not ok then warn("[MaruHub] Auto-load failed:", err) end
end)
...
-- FIX: Wrap hookfunction calls in pcall - crash if executor doesn't support
pcall(function()
...
local Fluent, SaveManager, InterfaceManager = loadstring(game:HttpGet("https://pastefy.app/DkAqkRMp/raw"))()
